...
const decryptFeistelV3 = (
  ...
  const innerFeistelResultC = innerFeistelFunction(ciphertextC, key);
  const productOfXorCB = xor(
    bitToBytes(splitIntoByte(innerFeistelResultC)),
    ciphertextB
  );

  const innerFeisterResultCB = innerFeistelFunction(productOfXorCB, key);
  const productOfXorCBA = xor(
    bitToBytes(splitIntoByte(innerFeisterResultCB)),
    ciphertextA
  );

  const innerFeistelResultCBA = innerFeistelFunction(productOfXorCBA, key);
  const productOfXorCBAD = xor(
    bitToBytes(splitIntoByte(innerFeistelResultCBA)),
    ciphertextD
  );

  return [
    new Uint8Array(productOfXorCBAD),
    new Uint8Array(productOfXorCBA),
    new Uint8Array(productOfXorCB),
    new Uint8Array(ciphertextC)
  ];
};
...
const encryptV2 = (plaintext, key, iter) => {
  ...
  for (let i = 0; i < splitPlaintext.length; i++) {
    ...
    const splittedPlaintext = splitBytesIntoBlock(
      plaintextByte,
      plaintextByte.length / 4
    );
    ...
    console.log(plaintextA, plaintextB, plaintextC, plaintextD);
    ...
    let roundKeys = []; // Store all round keys
...
const encrypt = (plaintext, key, rounds) => {
  ...
  for (let block of splitPlaintext) {
...
const decrypt = (ciphertext, key, rounds) => {
  const checkedKey = checkAndModifyKey(key);
  ...
  for (let block of splitCiphertext) {
    ...
    for (let i = 0; i < rounds; i++) {
      // feistel
      const feistelResult = decryptFeistelV3(
        cipherTextA,
        cipherTextB,
        cipherTextC,
        cipherTextD,
...
const decryptV2 = (ciphertext, key, rounds) => {
  const checkedKey = checkAndModifyKey(key);
  const splitCiphertext = splitPlaintextTo128Bit(ciphertext);
  ...
  for (let block of splitCiphertext) {
    ...
    for (let i = 0; i < rounds; i++) {
      // feistel
      const feistelResult = decryptFeistelV2(
        cipherTextA,
        cipherTextB,
        cipherTextC,
        cipherTextD,
...
const encryptionResult = encrypt(
  `Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla id eros augue. In hac habitasse platea dictumst. Ut interdum, turpis et volutpat vehicula, neque mauris auctor arcu, id tempor ipsum sem at erat. Vivamus elit sem, dapibus quis ligula non, luctus efficitur purus. Praesent vehicula mattis accumsan. Aenean lacinia tellus dolor, sit amet pharetra odio eleifend at. Integer pulvinar dapibus lobortis. Duis vestibulum risus sed mauris rhoncus, in tempus enim pretium. Donec id lacinia erat. Aliquam ultricies semper enim, quis commodo purus tincidunt quis. Duis cursus est et lacus auctor ultrices. Nunc sem sem, varius a ullamcorper eu, tristique quis orci.

  Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Integer sagittis suscipit lectus vel sagittis. Curabitur id consequat sem. Vivamus consequat mattis tincidunt. Curabitur eros est, pretium ut nisi et, accumsan porttitor risus. Vivamus nulla odio, consectetur ac est et, mattis ultricies leo. Donec quis ultrices nisi. Aliquam accumsan velit non lorem imperdiet aliquet. Nulla eu ultrices tortor. Donec at risus odio.
  
  Sed ut diam eget ex elementum porta. Nulla nec laoreet ex, eu efficitur nulla. Integer at lacus tellus. Etiam iaculis orci felis, id consectetur massa interdum eget. Ut facilisis massa lobortis nulla porta, eu convallis enim semper. Vestibulum hendrerit arcu vel nulla feugiat, ac pharetra neque dapibus. Vestibulum imperdiet ultricies tempor. Phasellus condimentum facilisis dictum. Quisque auctor sit amet eros at iaculis. Mauris at velit eros. Vestibulum dignissim dui ac ipsum dictum volutpat in sed mauris. Nam vehicula erat leo, in auctor libero rhoncus sed. Suspendisse sed sem ut nibh sodales sodales nec consequat nisi. Nunc eu ex velit. Phasellus egestas molestie dapibus.
  
  Integer cursus sapien at diam faucibus bibendum. Sed eu arcu finibus, venenatis elit eu, aliquet justo. Pellentesque mauris ante, dictum vitae quam eget, dictum tristique neque. Aenean pulvinar a nunc at consequat. Vivamus venenatis elit varius est tempor luctus. Duis placerat fermentum gravida. Vivamus tempor cursus sapien, in euismod quam finibus at. Donec nec purus sit amet leo finibus sollicitudin. Mauris rutrum nulla eu lorem auctor, nec sagittis augue cursus. Integer tristique leo rutrum, blandit urna in, venenatis ligula. Pellentesque placerat felis id neque tincidunt efficitur. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Mauris ac congue purus, vel rutrum risus. Aliquam consequat varius lacus vitae vulputate. Aenean nisl lectus, molestie eget tristique ac, finibus vestibulum mauris. Maecenas at ex lobortis, venenatis elit quis, rutrum eros.
  
  Integer tellus risus, ultricies vitae arcu et, dictum tincidunt neque. Quisque blandit libero sed neque iaculis, sit amet fringilla nunc cursus. Nam ut auctor elit, quis ultrices justo. Vestibulum faucibus nibh vitae metus faucibus malesuada. Quisque eu odio viverra, eleifend eros eu, commodo augue. Cras condimentum vehicula interdum. Vestibulum ut tincidunt elit. Praesent et dui non velit pellentesque varius. Maecenas nec laoreet ex, quis posuere odio. Aliquam mi ligula, faucibus vel ligula vel, aliquet pharetra urna.
  
  Integer vel ligula accumsan elit rutrum finibus. Aenean bibendum tempor magna, eu fringilla massa malesuada id. Sed ornare volutpat diam et facilisis. Donec ut turpis quam. Sed imperdiet gravida ipsum, sed aliquam ex imperdiet ut. Etiam faucibus sapien sed nisi hendrerit elementum. Nullam eget sollicitudin diam. Donec ut pellentesque orci. Fusce in est quis augue scelerisque egestas non vitae leo. Phasellus lectus orci, molestie nec odio in, pellentesque sodales mauris. Fusce molestie augue et arcu fermentum aliquam. Nunc non finibus tortor, ac sodales neque.
  
  Mauris ac tempor nisi. Phasellus id urna sit amet neque dapibus commodo. Sed nec nisi eget dui cursus pharetra. Proin vitae egestas lectus. In hac habitasse platea dictumst. Donec quis massa lectus. Cras sed dui metus. Fusce molestie risus in scelerisque accumsan. Curabitur a dignissim arcu, quis sollicitudin orci. Praesent varius eu quam vitae pulvinar. Duis facilisis sed neque quis blandit.
  
  Phasellus risus orci, egestas vel porta non, iaculis in odio. Nulla auctor aliquet libero eu euismod. Nunc ut varius metus, nec porta tellus. Nullam malesuada dolor ut aliquam lacinia. Curabitur dignissim lorem vel ex bibendum, at condimentum libero rhoncus. Nullam vestibulum nibh et ex lobortis fringilla ut id diam. Nunc luctus velit id tellus ornare, nec gravida elit sodales. Nam non lectus convallis, fermentum lorem sed, rhoncus sapien. Etiam at erat lacus. Vestibulum viverra at nibh eget maximus. Ut ultrices dignissim dictum. Aenean id ligula in nunc suscipit viverra.
  
  Etiam sed lacus quis velit porta pretium eu id lectus. Sed cursus ligula neque, sit amet mollis diam ultrices nec. Vestibulum vel mi id velit imperdiet ornare id id ex. Nullam et luctus dolor. Aenean a eros vitae magna rutrum vulputate ac eu magna. Praesent suscipit, orci eget lobortis mattis, orci sapien feugiat quam, sed ultrices lorem velit quis turpis. Sed mattis venenatis ultricies. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Vivamus mattis mauris in urna rhoncus luctus. Quisque congue viverra mauris vitae laoreet. Nullam vitae leo faucibus, cursus tellus sed, pellentesque nisl. Aliquam sit amet arcu est. Proin accumsan dictum fringilla. Pellentesque a luctus diam. Morbi et est egestas, condimentum elit eget, tincidunt magna. Vivamus ac nibh et dolor pretium vestibulum vitae sed est.
  
  Phasellus mollis viverra neque vitae fermentum. Pellentesque nec suscipit mauris. Quisque sed risus dolor. Phasellus in euismod ante, non dapibus lorem. Nam turpis neque, pharetra nec porttitor eu, vestibulum vitae eros. Pellentesque id lacus mollis, tincidunt lorem sed, volutpat nunc. Etiam tellus massa, pharetra sit amet diam eu, posuere pharetra sem. Integer in mauris tortor. Nullam ut egestas felis, vitae vehicula tortor. Aliquam a lacus leo. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia curae;`,
  "hello world",
  16
);
const decryptionResult = decryptV2(encryptionResult, "hello world", 16);

console.log(encryptionResult, "encrypt");
console.log(byteToString(encryptionResult));

console.log(decryptionResult, "decrypt");
console.log(byteToString(decryptionResult));
